// ROI : 내가 원하는 구역만 뽑아서 처리하는 방법
// 이진화를 나눈다는 건 백그라운드와 포그라운드를 나눈다느 것임
//Component Labelling
...

{
	...
	float grayGap = 255.0f / (float)curColor; // 255.0f : 덩어리가 25번까지 구현이 되어있다,

	// 가장 면적이 넓은 영역을 찾아내기 위함
	for (i = 1; i <= curColor; i++) // 0번 index가 없기때문에 ㅇ첫번째 인덱스의 레이블이 컴포넌트별로 픽셀의 단위로 나오게 되는 거임
	{
		if (BlobArea[i] >= BlobArea[Out_Area]) Out_Area = i; //동전 중에서 가장 큰 영역을 찾기위한과정
	}
	// CutImage 배열 클리어~
	for (k = 0; k < width * height; k++) CutImage[k] = 255; // 배경을 다 하얗게 채워줌

	// coloring에 저장된 라벨링 결과중 (Out_Area에 저장된) 영역이 가장 큰 것만 CutImage에 저장
	for (k = 0; k < width * height; k++)
	{
		if (coloring[k] == Out_Area) CutImage[k] = 0;  // 가장 큰 것만 저장 (size filtering) 아래 두개 코드 포함해서 세개중에 하나만 상요하면 된ㅁ다.
													   //if (BlobArea[coloring[k]] > 500) CutImage[k] = 0;  // 특정 면적이상되는 영역만 출력
													   //CutImage[k] = (unsigned char)(coloring[k] * grayGap); //라벨링된 결과를 있는 그대로 출력하기
	}
...

{
	...
	//가장 큰 영역을 가지고 있는 동전에 테두리, 경계선을 넣는 과정
	...
	for (int i = 0; i < H; i++) {
		...
		for (int j = 0; j < W; j++) {
			...
			if (Temp[i*W+j] == 0) {
				...
				if (!(Temp[(i-1)*W+j] == 0 && Temp[(i + 1)*W + j] == 0 && Temp[i*W+(j-1)] == 0 && Temp[i*W + (j + 1)] == 0)) { //경계인것을 찾는 것임
	...
	Binarization(Image, Temp, W, H, 40); //이진화 임계치를 80으로 잡으면 속눈썹과 동공이 연결되어버림

	InverseImage(Temp, Temp, W, H);

	m_BlobColoring(Temp, H, W);

	SaveBMPFile(hf, hInfo, hRGB, Temp, hInfo.biWidth, hInfo.biHeight, "output.bmp");

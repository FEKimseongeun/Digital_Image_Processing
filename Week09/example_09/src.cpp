	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			if ((i + Ty < H && i + Ty >= 0) && (j + Tx < W && j + Tx >= 0))
				...
				Output[(i + Ty) * W + (j + Tx)] = Image[i * W + j]; // 인
		}
	}
}

void Scaling(BYTE* Image, BYTE* Output, int W, int H, double SF_X, double SF_Y) // 포워드 매핑과 백워드 매핑이 필요함.
{
	// Scaling
	int tmpX, tmpY;
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			//int 자료형변환을 해준채로 나눗셈을 사용한 이유는 이렇게  안해주게 되면 x화소로 가로로 벌릴때 구멍이 생기는 문제가 발생 0, 1, 2, 3, 4, 5....에 1.5를 곱해주면 0 1 3 4 ... 이런식으로 계산이 되어서 "2"가 비게되느 문제 발생
			//1.5로 곱하는게 아니라 나누게 되면 해결이 된다.
			tmpX = (int)(j / SF_X);
			tmpY = (int)(i / SF_Y);
			if (tmpY < H && tmpX < W)//클리핑
				//출력 영상을 다 훑으면서 역으로 검사래나가는 방식
				Output[i * W + j] = Image[tmpY * W + tmpX];//X,Y위치에 있던 걸 tmpX, tmpY위치로 보내겠다는 코드
			//기하변환을 역으로 두면 원래대로 복귀하는 방식이된다.
		}
	}
...
{
	int tmpX, tmpY;
	double Radian = Angle * 3.141592 / 180.0;//디그리를 라디안으로 변환해주는 방식
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
...
int main()
{
	...
	//Translation(Image, Output, W, H, 100, 40); //이것만 돌릴경우 가로 세로 이동이 잘되긴 하는데 BMP파일이 원본소스가 뒤집혀 있다는 특징때문에 원점이 왼쪽 위가 아니라 왼쪽 아래로 잡혀서 아래에서 위로 이동한 결가가 났다고 볼 수 있음.
